...
void DisplayData::downloadResult(QNetworkReply* reply)
{
    ...
    if (json["status"] == true) {
        ...
        ui->lb_current_hashrate->setText(json["data"].toObject()["hashrate"].toString());
        ...
        for (int i = 0; i < workers.count(); ++i) {
            ...
            ui->tbl_worker->setItem(i, 1, new QTableWidgetItem(worker["lastshare"].toDouble()));
            ui->tbl_worker->setItem(i, 2, new QTableWidgetItem(worker["rating"].toDouble()));
            ui->tbl_worker->setItem(i, 3, new QTableWidgetItem(worker["hashrate"].toString()));
        }
